...
class DataLoader:
    ...
    def __iter__(self):
        ...
        ########################################################################
        if self.shuffle:
            random_index = iter(np.random.permutation(self.dataset.__len__()))
        else:
            random_index = iter(range(self.dataset.__len__()))
        batch = []
        
        for index in random_index:
            batch.append(self.dataset[index]["data"])
            if batch.__len__() == self.batch_size:
                yield dict({"data":np.array(batch)})
                batch = []
                
        if self.__len__() * self.batch_size > self.dataset.__len__():
            yield dict({"data":np.array(batch)})
